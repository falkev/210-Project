...
pub fn read_cost_index_csv(file_path: &str) -> Result<Vec<(String, f64)>, Box<dyn Error>> {
    ...
    for (index, line) in reader.lines().enumerate() {
        ...
        let cost_index: f64 = parts[1].trim().parse()?;

        data.push((country, cost_index));
    }

    Ok(data)
}

pub fn visualize_cost_index_graph(
    tourism_data: Vec<(String, f64)>,
    cost_index_data: Vec<(String, f64)>,
    threshold: f64,
    output_file: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    let cost_index_map: HashMap<_, _> = cost_index_data.into_iter().collect();
    let combined_data: Vec<(String, f64)> = tourism_data
        .into_iter()
        .filter_map(|(country, tourism)| {
            cost_index_map.get(&country).and_then(|&cost_index| {
                if cost_index > 0.0 && tourism > 0.0 {
                    Some((country, tourism / cost_index))
                } else {
                    None
                }
            })
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption("Tourism-to-Cost Index Graph", ("sans-serif", 30))
    ...
    chart.draw_series(std::iter::once(Text::new(
        "Node size: Tourism-to-Cost Index ratio\nEdge thickness: Similarity",
        (-200, 220),
        ("sans-serif", 12),
    )))?;
